# data-validation-{{cookiecutter.repo_name}} <!-- omit in toc -->
...
### Usage
```sh
$ python -m data_validation_{{cookiecutter.module_name}}.validation
```
...
`/etc/data-validation-{{cookiecutter.repo_name}}-conf.ini`. 
...
The application must be granted read access to the egress dataset on 
